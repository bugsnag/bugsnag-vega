...
describe('app plugin', () => {
  it('syncs app info with native layer', () => {
    const setApp = BugsnagKeplerNative.setApp as jest.Mock
    ...
    expect(setApp).toHaveBeenCalledWith({
      id: 'com.bugsnag.fixtures.keplertestapp',
      type: 'test-app',
      version: '1.2.3',
      releaseStage: 'test',
      binaryArch: 'amd64'
  ...
  it('sets app data in events', () => {
    ...
    expect(payloads[0].events[0].app.id).toBe('com.bugsnag.fixtures.keplertestapp')
